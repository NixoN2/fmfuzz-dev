...
# Import config module for solver configuration
...
class PrepareCommitAnalyzer:
    def __init__(self, repo_path: str = ".", compile_commands: Optional[str] = None, solver_name: str = ""):
        """Initialize with repository path and solver name."""
        ...
        self.clang_config = self.solver_config.get("clang_config", {})
        self.namespace_prefix = self.clang_config.get("namespace_prefix", "")
    ...
    def get_function_signature(self, cursor) -> Optional[str]:
        ...
        Uses c++filt demangling of the mangled name, which directly matches
        the format produced by gcov coverage output.
        """
        try:
    ...
    def get_qualified_name(self, cursor) -> str:
        ...
        # Normalize namespace prefix if configured
        if self.namespace_prefix:
            ns = self.namespace_prefix.rstrip("::")
            if not qualified_name.startswith(self.namespace_prefix):
                if self.namespace_prefix in qualified_name:
                    idx = qualified_name.find(self.namespace_prefix)
                    qualified_name = qualified_name[idx:]
        
        return qualified_name
    
    def is_solver_function(self, signature: str) -> bool:
        """Check if a function signature belongs to the configured solver.
        ...
        try:
            ...
            if head.startswith('std::') or head.startswith('__') or head.startswith('__gnu_cxx::'):
                return False
            if self.namespace_prefix and self.namespace_prefix in head:
                return True
            if '::' in head:
                ns = head.split('::', 1)[0]
    ...
    def get_commit_functions(self, commit_hash: str) -> tuple[List[str], List[str]]:
        ...
        for file_path, changed_lines in changed_files_lines.items():
            ...
            if after_funcs:
                solver_funcs = [f for f in after_funcs if self.is_solver_function(f.signature)]
                for ln in sorted(changed_lines):
                    candidates = [f for f in solver_funcs if int(f.start) <= ln <= int(f.end)]
                    if not candidates:
                        continue
    ...
    def parse_functions_from_text(self, file_path: str, source_text: Optional[str]) -> List[FunctionInfo]:
        ...
        try:
            ...
            all_funcs_count = 0
            solver_funcs_count = 0
            unknown_kinds_count = 0

            def visit(n):
                nonlocal all_funcs_count, solver_funcs_count, unknown_kinds_count
                try:
                    ...
                    if cursor_kind in [clang.cindex.CursorKind.FUNCTION_DECL, clang.cindex.CursorKind.CXX_METHOD,
                        ...
                        if is_def:
                            ...
                            if sig:
                                is_solver = self.is_solver_function(sig)
                                if is_solver:
                                    solver_funcs_count += 1
                                if sig and node_file and is_solver:
    ...
    def _build_clang_args(self) -> List[str]:
        """Build unified clang arguments from solver config.
        Note: We're parsing for AST analysis, not building, so we don't need coverage/debug flags.
        """
        cc = self.clang_config
        cpp_std = cc.get("cpp_standard", "c++20")
        ...
        args.extend(cc.get("includes", []))
        
        # Preprocessor defines from config
        args.extend(cc.get("defines", []))
        
        # Common compiler flags for parsing
        args.extend([
            '-fPIC',
            '-Wall',
            '-Wsuggest-override',
        ...
        # Extra solver-specific flags from config
        args.extend(cc.get("extra_flags", []))
...
def main():
    ...
    parser.add_argument('--solver', required=True, help='Solver name (e.g., z3, cvc5)')
    ...
    # Initialize analyzer
    analyzer = PrepareCommitAnalyzer(".", compile_commands=args.compile_commands, solver_name=args.solver)
